    ...
    for cocktail_set in cocktail_scan:
        ...
        for noise in noise_scan:
            ...
            mi_x, mi_y, mi_z = generate_svg_plots([a1, a2, a3], colors, ID=ID)
                # Alternatively:
                # mi_x, mi_y, mi_z = compute_mutual_information(a_list=[a1, a2, a3])
                
                # mi_x_list.append(mi_x)
                # mi_y_list.append(mi_y)
                # mi_z_list.append(mi_z)
            
            # If datasets already exist for this group, delete them first
            # for dset in ["NS_ratios", "mutual_x", "mutual_y", "mutual_z"]:
            #     if dset in group:
            #         del group[dset]
            
            # Create datasets under the group
            # group.create_dataset("NS_ratios", data=ns_list)
            # group.create_dataset("mutual_x", data=mi_x_list)
            # group.create_dataset("mutual_y", data=mi_y_list)
            # group.create_dataset("mutual_z", data=mi_z_list)
